class Users::RegistrationsController < Devise::RegistrationsController
  ...
  before_filter :init_admin_data, only: [:new, :create]
  ...
  private
    def init_admin_data
      return_available_profiles
      @class_dashboard = "active"
    end

    # A user may create profiles equals or under his own
    def return_available_profiles
      profile = current_user.profile.shortcut rescue ""
      case profile
        when "ADM"
          @profiles = Profile.where(shortcut: ["ADM", "AUD"])
        when "AUD"
          @profiles = Profile.where(shortcut: ["AUD"])
        when "S-ADM"
          @profiles = Profile.where(shortcut: ["S-ADM", "ADM", "AUD"])
        else
          #@profiles = Profile.where(shortcut: ["S-ADM", "ADM", "AUD"])
          @profiles = []
        end
